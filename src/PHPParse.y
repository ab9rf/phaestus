...
namespace_name:
      IDENT                      { [$1] }
   |  namespace_name '\\' IDENT  { $3:$1 }
;

top_statement:
      statement                        { Statement $1 }
   |  function_declaration_statement   { FunctionDeclaration $1 }
   |  class_declaration_statement      { ClassDeclaration $1 }
   |  T_NAMESPACE namespace_name ';'   { NamespaceD $2 }
   |  T_NAMESPACE namespace_name '{' top_statement_list '}'   
                                       { Namespace $2 (reverse $4) }
   |  T_NAMESPACE '{'   top_statement_list '}'   
                                       { NamespaceG (reverse $3) }
   |  T_USE use_declarations ';'       { UseDeclaration (reverse $2) }
   |  constant_declaration ';'         { ConstantDeclarations (reverse $1) }
;
...
use_declaration:
   ...
   |  '\\' namespace_name              { UseAbsolute $2 }
   |  '\\' namespace_name T_AS IDENT   { UseAbsoluteAs $2 $4 }
;

constant_declaration:
      constant_declaration ',' IDENT '=' static_scalar   
                                       { (ConstantDeclaration $3 $5):$1 }
...
finally_statement:
   {- empty -}   { Nothing } 
   |  T_FINALLY  '{' inner_statement_list '}'   { Just $3 }   
;
...
non_empty_parameter_list:
   optional_class_type T_VARIABLE   { [ParameterDef $1 $2] }   
   |  optional_class_type '&' T_VARIABLE   { [RefParameterDef $1 $3] }
   |  optional_class_type '&' T_VARIABLE '=' static_scalar   { [RefParameterDefWithDefault $1 $3 $5] }
   |  optional_class_type T_VARIABLE '=' static_scalar   { [ParameterDefWithDefault $1 $2 $4] }
   |  non_empty_parameter_list ',' optional_class_type T_VARIABLE   { (ParameterDef $3 $4):$1 }
   |  non_empty_parameter_list ',' optional_class_type '&' T_VARIABLE   { (RefParameterDef $3 $5):$1 }
   |  non_empty_parameter_list ',' optional_class_type '&' T_VARIABLE   '=' static_scalar   { (RefParameterDefWithDefault $3 $5 $7):$1 }
   |  non_empty_parameter_list ',' optional_class_type T_VARIABLE '=' static_scalar   { (ParameterDefWithDefault $3 $5 $6):$1 }
;
...
non_empty_function_call_parameter_list:
   expr_without_variable   { [Parameter $1] }
   |  variable   { [Parameter $1] }
   |  '&' w_variable   { [RefParameter $2] }
   |  non_empty_function_call_parameter_list ',' expr_without_variable   { (Parameter $3):$1 }   
   |  non_empty_function_call_parameter_list ',' variable   { (Parameter $3):$1 }   
   |  non_empty_function_call_parameter_list ',' '&' w_variable   { (RefParameter $4):$1 }   
;
...
global_var:
   T_VARIABLE   { GlobalVar $1 }   
   |  '$' r_variable   { IndirectGlobalVar $2 }
   |  '$' '{' expr '}'   { IndirectGlobalVar $3 }
;
...
class_statement_list:
   class_statement_list class_statement   { $2:$1 }
   |  {- empty -}   { [] }
;
...
class_statement:
   ...
   |  class_constant_declaration ';'   { ClassConstantDeclaration $1 }
...
non_empty_trait_adaptation_list:
   ...
   |  non_empty_trait_adaptation_list trait_adaptation_statement   { $2:$1 }
;
...
chaining_dereference:
   chaining_dereference '[' dim_offset ']'   { $3:$1 }
   |  '[' dim_offset ']'   { [$2] }
;
...
expr_without_variable:
   ...
   |  variable '+=' expr   { AddInto $1 $3 }
   |  variable '-=' expr   { SubtractInto $1 $3 }
   |  variable '*=' expr   { MultiplyInto $1 $3 }   
   |  variable '/=' expr   { DivideInto $1 $3 }   
   |  variable '.=' expr   { ConcatInto $1 $3 }
   |  variable '%=' expr   { ModulusInto $1 $3 }   
   |  variable '&=' expr   { AndInto $1 $3 }   
   |  variable '|=' expr   { OrInto $1 $3 }   
   |  variable '^=' expr   { XorInto $1 $3 }   
   |  variable '<<=' expr   { ShiftLeftInto $1 $3 }
   |  variable '>>=' expr   { ShiftRightInto $1 $3 }
   |  rw_variable '++'   { Postincrement $2 }
   |  '++' rw_variable   { Preincrement $2 }
   |  rw_variable '--'   { Postdecrement $2 } 
   |  '--' rw_variable   { Predecrement $2 }
   |  expr '||'  expr   { BooleanOr $1 $3 }
   |  expr '&&' expr   { BooleanAnd $1 $3 } 
   |  expr 'or'  expr   { LogicalOr $1 $3 }
   |  expr 'and' expr   { LogicalAnd $1 $3 }
   |  expr 'xor' expr   { LogicalXor $1 $3 }
   ...
   |  expr '!==' expr   { IsNotIdentical $1 $3 }
   |  expr '==' expr   { IsEqual $1 $3 }   
   |  expr '!=' expr   { IsNotEqual $1 $3 }   
   |  expr '<' expr   { LessThan $1 $3 }   
   |  expr '<=' expr { LessThanOrEqual $1 $3 }
   |  expr '>' expr   { GreaterThan $1 $3 }   
   |  expr '>=' expr { GreaterThanOrEqual $1 $3 }
   |  expr 'instanceof' class_name_reference   { InstanceOf $1 $3 }
   |  parenthesis_expr   { $1 }
   |  new_expr   { $1 }
...
yield_expr:
   ...
   |  T_YIELD expr '=>' expr_without_variable   { Yield2 $2 $4 } 
   |  T_YIELD expr '=>' variable   { Yield2 $2 $4 } 
;

combined_scalar_offset:
   combined_scalar '[' dim_offset ']'   { ScalarWithOffset (SWOArray $1) [$3] }
   | combined_scalar_offset '[' dim_offset ']'   { (\(CombinedScalarWithOffset x [y]) -> CombinedScalarWithOffset x [$3:y]) $1 }
   | T_STRING_CONST '[' dim_offset ']'   { ScalarWithOffset (SWOString $1) [$3] }
...
lexical_var_list:
   lexical_var_list ',' T_VARIABLE   { (LexicalVariable $3):$1 }   
   |  lexical_var_list ',' '&' T_VARIABLE   { (LexicalVariableRef $4):$1 }   
   |  T_VARIABLE   { [LexicalVariable $1] }   
   |  '&' T_VARIABLE   { [LexicalVariableRef $2] }   
;

function_call:
   namespace_name function_call_parameter_list   { FunctionCall (RelativeNamespace $1) $2 }
   |  T_NAMESPACE '\\' namespace_name function_call_parameter_list   { FunctionCall (SelfNamespace $3) $4 }
   |  '\\' namespace_name function_call_parameter_list   { FunctionCall (AbsoluteNamespace $2) $3 }
...
class_name:
   ...
   |  T_NAMESPACE '\\' namespace_name   { SelfNamespace $3 }
   |  '\\' namespace_name   { AbsoluteNamespace $2 }
;
...
common_scalar:
   ...
   |  T_STRING_CONST   { Constant $1 }
   |  T_LINE   { MagicLine }   
   |  T_FILE   { MagicFile }   
   |  T_DIR   { MagicDir }   
   |  T_TRAIT_C   { MagicTrait }   
   |  T_METHOD_C   { MagicMethod }   
   |  T_FUNC_C   { MagicFunc }   
   |  T_NS_C   { MagicNamespace }   
...
static_scalar: 
   ...
   |  namespace_name   { NamespaceScalar (RelativeNamespace $1) }
   |  T_NAMESPACE '\\' namespace_name { NamespaceScalar (SelfNamespace $3) }
   |  '\\' namespace_name   { NamespaceScalar (AbsoluteNamespace $2) }
   |  '+' static_scalar   { StaticUnaryPlus $2 }
   |  '-' static_scalar   { StaticUnaryMinus $2 }
   |  T_ARRAY '(' static_array_pair_list ')' { StaticArray $3 }
   |  '[' static_array_pair_list ']'   { StaticArray $2 }
   |  static_class_constant   { $1 }
   |  T_CLASS_C   { MagicClass }
;

static_class_constant:
   class_name '::' IDENT   { StaticClassConstant $1 $3 }
;

scalar:
   T_VARIABLE_STR   { Variable $1 }   
   ...
   |  '\\' namespace_name   { NamespaceScalar (AbsoluteNamespace $2) }
   |  common_scalar   { $1 }
   |  '"' encaps_list '"'   { ScalarString $2 }
   |  T_START_HEREDOC encaps_list T_END_HEREDOC { ScalarString $2 }
   |  T_CLASS_C   { MagicClass $1 }
;
...
non_empty_static_array_pair_list:
   non_empty_static_array_pair_list ',' static_scalar '=>' static_scalar   { (StaticArrayPairKV $3 $5):$1 }   
   |  non_empty_static_array_pair_list ',' static_scalar   { (StaticArrayPairV $1):$3 }
   |  static_scalar '=>' static_scalar   { [StaticArrayPairKV $1 $3] }
   |  static_scalar   { [StaticArrayPairV $1] }
;
...
variable:
   ...
   |  base_variable_with_function_calls   { VariableWithoutArrow $1 }
;
...
variable_property:
   '->' object_property  method_or_not   { VariableProperty $1 $2 }
;
...
method_or_not:
   method   { Method $1 }   
   |  array_method_dereference   { ArrayMethod $1 }
   |  {- empty -}   { NotMethod } 
;

variable_without_objects:
   reference_variable   { VariableWithoutObjects 0 $1 }
   |  simple_indirect_reference reference_variable   { VariableWithoutObjects $1 $2 }
;
...
base_variable:
   reference_variable   { BaseVariable 0 $1 }
   |  simple_indirect_reference reference_variable   { BaseVariable $1 $2 } 
   |  static_member   { $1 }
;

reference_variable:
   reference_variable '[' dim_offset ']'   { (\(ReferenceVariable v x) -> ReferenceVariable v ($3:(RVOffset x))) $1 }
   |  reference_variable '{' expr '}'   { (\(ReferenceVariable v x) -> ReferenceVariable v ($3:(RVIndex x)))  $1 }
...
encaps_list:
   ...
   |  encaps_list T_STRING_CONST   { (Encaps $2) : $1 }
   |  encaps_var   { (EncapsVar $1) : [] }
   |  T_STRING_CONST encaps_var   { (Encaps $1) : [] }
;

encaps_var:
   T_VARIABLE   { Variable $1 }
   |  T_VARIABLE '['  encaps_var_offset ']'   { VariableOffset $1 $3 }
   |  T_VARIABLE '->' IDENT   { VariableProperty $1 $3 } 
   |  '${' expr '}'   { EncapsExpr $2 }
   |  '${' T_VARIABLE_STR '[' expr ']' '}' 
   { VariableOffsetE $2 $4 }
   |  '{' variable '}'   { VariableAlt $2 }
;
