...
token0 =
 ...
 where
  start' =
    (try startEcho >> next tokenPhp >> return [KeywordEcho])
...
tokenPhp =
 ...
 where
  ...
  rbrace = PC.char '}' >> do
    ...
    go' RBrace
   where
    pop (ParserState [s    ]) = ParserState [s]
  ...
  int =
    (try bin <|> try hex <|> try oct <|> dec) >>= \s -> go' $ IntegerToken s

  dec =
    PC.string "0" <|> PC.oneOf ['1' .. '9'] >:+> many (PC.oneOf ['0' .. '9'])
  hex = PC.char '0' >:+> PC.oneOf "xX" >:+> many1
    (PC.oneOf $ ['0' .. '9'] ++ ['a' .. 'f'] ++ ['A' .. 'F'])
  bin = liftM3 (\a b c -> a : b : c)
               (PC.char '0')
               (PC.oneOf "bB")
               (many1 (PC.oneOf "01"))
  ...
  herenow labelP = do
   ...
   where
      ...
      nl >> PC.string lbl >> lookAhead (try (optional (PC.char ';') >> nl))

  hereDoc = do
    (bflag, lbl, end) <- herenow
      (try ident <|> between (PC.char '"') (PC.char '"') ident)
    go (tokenHd end) (StartHereDoc bflag lbl)
   where
    tokenHd end = shift (interpolated "" (try end >> go tokenPhp EndHereDoc))
  ...
  sqStr = do
    ...
    sval  <- between
      ...
      (PC.char '\'')
      (many
        ((PC.char '\\' >> (PC.oneOf "'\\" <|> return '\\')) <|> PC.noneOf "'")
      )
  ...
  tokenDq = shift
    $ interpolated "\"" (PC.char '"' >> go tokenPhp EndInterpolatedString)
 ...
 where
  i' =
    (eof >> return [])
      <|> try end
      <|> try
            (   between (PC.string "${") (PC.string "}") ident
            >>= go'
      ...
      <|> try
            (   PC.char '$'
            >>  ident
            >>= go (interpolated' (interpolated q end))
            .   InterpolatedVariable
            )
      <|> try
            (  PC.char '{'
...
intParser base m' = PC.oneOf digits >>= (p (pred m') . value)
 where
  ...
  p 0 i = return i
  p m i = (try (PC.oneOf digits) >>= (\j -> p (pred m) ((i * base) + value j)))
    <|> return i
...
interpolated' ret =
  try (PC.string "->" >> ident >>= (go' . InterpolatedProperty))
    <|> try
          (   between (PC.string "[") (PC.string "]") ident
          >>= (go' . InterpolatedIndexIdent)
          )
    <|> try
          (   between (PC.string "[") (PC.string "]") lnum
          >>= (go' . InterpolatedIndexInt)
          )
    <|> shift ret
